/**
 * $Id: VmVarI18nInfo.java,v 1.1 2012/09/11 06:37:55 chenhua Exp $
 */
...
public class VmVarI18nInfo {
    ...
    public VmVarI18nInfo(IPEntity pEntity) {
        String modelName = StringUtil.lowerFirst(pEntity.getName());
        //获取显示模型的设置的title
        Map zhTitleMap = new HashMap();
        Map enTitleMap = new HashMap();
        setTitleMap(pEntity.getResultForm(), zhTitleMap, enTitleMap, modelName+".list.title", modelName+" list");
        setTitleMap(pEntity.getNewForm(), zhTitleMap, enTitleMap, modelName+".form.title", modelName+" input");
        setTitleMap(pEntity.getDetailInfoForm(), zhTitleMap, enTitleMap, modelName+".show.title", modelName+" show");
        addZhFields(zhTitleMap);
        addEnFields(enTitleMap);
        //获取显示模型设置的字段信息
        Collection fields = pEntity.getFields();
        Collection queryFormFields = pEntity.getQueryForm().getFields();
        Collection queryResultFields = pEntity.getResultForm().getFields();
        Collection newFormFields = pEntity.getNewForm().getFields();
        Collection detailInfoFields = pEntity.getDetailInfoForm().getFields();
