/*
 * $Id: GenHbm.java,v 1.7 2013/04/19 09:51:24 zhanghp Exp $
 *
...
public class GenHbm {
	...
	public static List genHbmFile(Collection pEntitys, String packageName,
		...
		for (Iterator iterator = pEntitys.iterator(); iterator.hasNext();) {
			...
			genIdAndPropertyElement(classElement, iPEntity, packageName);
	 ...
	 */
	public static void genHbmFile(Collection pEntitys, Map autoCodeInfoMap) {
		if (pEntitys == null || pEntitys.isEmpty()) {
			log.error("持久化实体集合不能为空");
		}
		for (Iterator iterator = pEntitys.iterator(); iterator.hasNext();) {
			IPEntity iPEntity = (IPEntity) iterator.next();
			AutoCodeInfo autoCodeInfo = autoCodeInfoMap.get(iPEntity.getName()) == null ? null
					: (AutoCodeInfo) autoCodeInfoMap.get(iPEntity.getName());
			genHbmFile(iPEntity, autoCodeInfo);
		}
	}
	 ...
	 * @param autoCodeInfo
	 */
	public static void genHbmFile(IPEntity iPEntity, AutoCodeInfo autoCodeInfo) {
		if (StringUtil.isNotBlank(autoCodeInfo.getHbmPath())) {
			Document hbmDocument = HbmUtil.createHbmDocument();
			Element classElement = genClassElement(
					hbmDocument.addElement(elementRoot), iPEntity,
					autoCodeInfo.getPoPackageName());
			setSqlElement("list", "default", iPEntity.getQuerySql(),
					classElement);
			genIdAndPropertyElement(classElement, iPEntity,
					autoCodeInfo.getPoPackageName());
			genRelationsElement(classElement, iPEntity,
					autoCodeInfo.getPoPackageName());
			File hbmFile = FileUtil.createFile(autoCodeInfo.getHbmPath());
			XmlUtil.saveToFile(hbmDocument, hbmFile);
			log.debug("gen " + iPEntity.getName() + " hbm file success");
		}
	}
	...
	public static void genIdAndPropertyElement(Element element,
			IPEntity pEntity, String packageName) {
		...
		if (pks == null || pks.isEmpty()) {
			...
			if (pks.size() == 1) {
				...
				setColumnProperty(pField, idElement);
				...
				for (Iterator iterator = pks.iterator(); iterator.hasNext();) {
					...
					keyPropertyElement.addAttribute("type",
							pField.getFieldType());
					setColumnProperty(pField, keyPropertyElement);
				}
			}
		}
		...
		for (Iterator iterator = fields.iterator(); iterator.hasNext();) {
			...
			setColumnProperty(pField, propertyElement);
